...
import pri.zxw.library.exception.CustomException;
...
//		mLabels = a.getTextArray(R.styleable.TabWidget_bottom_labels);
		mDrawableIds = HomeMapTool.getInstance().getBottomDraw();
		mLabels=HomeMapTool.getInstance().getBottomName();
		if (null == mLabels || mLabels.size() <= 0) {
			try {
				...
				Log.i(TAG, MyTabWidget.class.getSimpleName() + " 出错");
			}
			a.recycle();
			return;
		}
	...
	private void init(final Context context) {
		...
		for (int i = 0; i < size; i++) {
			...
			itemName.setCompoundDrawablesWithIntrinsicBounds(null, context
					.getResources().getDrawable(mDrawableIds.get(i)), null, null);
			itemName.setText(mLabels.get(i));
	...
	public void setTabsDisplay(Context context, int index) {
		...
		for (int i = 0; i < size; i++) {
			...
			if ((Integer) (checkedTextView.getTag()) == index) {
				Log.i(TAG, mLabels.get(i) + " is selected...");
	...
	/**
	 * 
	* @ClassName: OnTabSelectedListener
	* @Description:获取tab选中状态的接口
	* @author Lix
	* @date 2015-1-13 上午11:15:56
	*
	 */
