...
//			builder.addPart(fileName, new FileBody((File) entry.getValue()));
//		}
//		Map<String, FileByteEntity> fileByteUpload = ((MultiPartRequest) request).getFileByteUploads();
//		for (Map.Entry<String, FileByteEntity> entry : fileByteUpload.entrySet()) {
//			if(fileName==null||fileName.length()==0)
//				fileName=entry.getKey();
//			builder.addPart(fileName,
//					new ByteArrayBody( entry.getValue().fileByte,entry.getValue().fileName));
//		}
...
//				StringBody stringBody=	new StringBody("fdsfsdf", contentType);
